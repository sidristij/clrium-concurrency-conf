...
Первая группа - это код с блокировкой вызывающего потока. К этой гурппе относятся примеры (1-4). Втрорая группа (5-6) - это код без блокировок. При написания такого кода возникает вопрос, когда вызов `ConfigureAwait(false)` оправдан?  
Начнем с первой группы - код с блокировкой вызывающего потока.  
...
Но все становится гораздо хуже, когда мы работаем в окружении, в котором существует контекстом синхронизации отличный от стандартного. При наличии *особого* контекста синхронизации, смешивание синхронного и асинхронного кода, т.е. код с блокировкой, повышает  вероятность возникновения deadlock'ов многократно.  
...
Для того, чтобы понять причину возникновения deadlock'a, нужно проанализировать код конечного автомата, и далее код классов MS, в который преобразуется вызов async метода. В статье ["Async Await and the Generated StateMachine"](https://www.codeproject.com/Articles/535635/Async-Await-and-the-Generated-StateMachine) приводится подробный пример кода конечного автомата.  
Не буду приводить полный код конечного автомата для примера (7), покажу лишь важные для дальнейшего разбора строки:
```C#
//...
...
Обратите внимание на то, что в `AwaitUnsafeOnCompleted` передается taskAwaiter полученный от вунтреннего (относительно `FooAsync`) асинхронного вызова (`Delay`).
...
```
Вернемся ко второй группе примереров кода (5-6), и вопросу когда  оправдан вызов `ConfigureAwait(false)`? Как уже не раз отмечалось при разбора примера (7), конфигурировать объект ожидания надо там, где на его основе будет построено продолжение выполнения. Т.е. конфигурация требуется (если вы приняли такое решение) только **внутри** асинхронных методов. 
...
##### Для разработчиков клиентского кода:
 ...
 2. Если приходится делать блокировку, то определите, в каком окружении выполняется код: 
    ...
    * Если контекста синхронизации "нет", то: Какова будет нагрузка? Какова вероятность что ваша блокировка прведет к "утечки" потоков из пула? Хватит ли того числа потоков, что созданных на старте по умолчанию, или надо выделить больше?
