...
Первая группа - код с блокировкой вызывающего потока. К этой группе относятся примеры (1-4). Вторая группа (5-6) - код без блокировок. При написания такого кода возникает вопрос, когда вызов `ConfigureAwait(false)` оправдан?  
