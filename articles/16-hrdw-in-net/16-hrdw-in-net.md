...
Для действительно маленьких циклов, этот код выполняется немного медленнее. Но это нормализуется для входных данных уже из восьми эхлементов, после чего скорость выполнения начинает расти (занимая на 26% времени меньше для коллекции из 32 тыс. элеметов).  Стоит отметить, что такая оптимизация не всегда повышает производительность. К примеру при работе с коллекциями с элементами типа `float` "развернутая" версия алгоритма имеет практически туже скорость что и исходная, по этому очень важно проводить профилирование.
